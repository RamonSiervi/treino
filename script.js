/* =========================================================
   PERSONAL v8.4 — script.js
   Ramon — Linha do Tempo + Treinos + Timer + Histórico
   ========================================================= */

const LS_KEYS = {
  DONE: 'personal_done_v84',
  AVATAR: 'personal_avatar_v84',
  HISTORY: 'personal_history_v84'
};
...
function renderSections(){
  ...
  TREINOS.forEach(sec=>{
    ...
    sec.itens.forEach((it,i)=>{
      ...
      // Cabeçalho do card com o check à direita (NÃO sobrepõe no mobile)
    ...
    headerBtn.addEventListener('click', ()=>{
      ...
      if (open) tip('Dica rápida: 3–5 min de aquecimento antes de iniciar 🔥');
    });
...
// ------------ CONCLUÍDO & HISTÓRICO ------------
let doneState = loadJSON(LS_KEYS.DONE, {});
...
function initAssistant(){
  ...
  setTimeout(()=>tip('Postura e controle do movimento primeiro, carga depois.'),3800);
}
function init(){
  renderSections();
  buildTimelineSkeleton(); updateTimeline(); updateMiniStats();
  initAssistant();
}
