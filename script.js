/* ========= STATE / CONST ========= */
const LS_KEYS = {
  DONE: 'personal_done_v82',
  AVATAR: 'personal_avatar_v82',
  GOAL: 'personal_goal_v82',
  HISTORY: 'personal_history_v82'
};
const DEFAULT_GOAL = 25;

/* Exercícios e vídeos (todos na ordem enviada) */
const WORKOUTS = [
  {
    colorClass: 'sec-blue',
...
function logHistory(){ // cada conclusão soma 1 no dia
...
function refreshSummary(){
  ...
  for(let i=6;i>=0;i--){
    ...
    // cor por intensidade (sem números)
    const ratio = Math.min(count/goal, 1);
    const low = [232,238,252];  // azul clarinho base
    const high = [47,128,237];  // azul forte
    const mix = (a,b,t)=> Math.round(a+(b-a)*t);
    const r = mix(low[0], high[0], ratio);
    const g = mix(low[1], high[1], ratio);
    const b = mix(low[2], high[2], ratio);
...
function showTimer(seconds, title='Intervalo'){
  ...
  timer.running=false; timer.started=false; clearInterval(timer.intId); timer.intId=null;
...
btnConclude.addEventListener('click', ()=>{
  clearInterval(timer.intId); timer.intId=null; timer.running=false;
...
// Dicas iniciais (curtas e não-intrusivas)
